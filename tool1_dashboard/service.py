...
class Tool1Service:
    ...
    def _episode_retry_single_translation(self, episode_id: str, lang: str) -> None:
        ...
        try:
            ...
            diagnostics = self._normalize_translation_diagnostics(
                result,
            ...
            self.db.update_episode_language_status(
                ...
                script_path=None,
                spoken_script_path=None,
                error_message=self._translation_diagnostic_summary(diagnostics, fallback=str(exc)[:500])[:500],
            )
    ...
    def _translate_single_language(self, lang: str, ctx: TranslationContext) -> bool:
        ...
        review_enabled = self._translation_ai_review_enabled(ctx.settings)
        result = None
        try:
            ...
            if result is None:
                write_json(self._translation_log_path(ctx.workspace, lang), [])
                review_path = self._translation_review_path(ctx.workspace, lang)
                if review_path.exists():
                    review_path.unlink()
            write_json(self._translation_diagnostics_path(ctx.workspace, lang), diagnostics)
            self.db.update_episode_language_status(
                ctx.episode_id, lang,
                translation_status="failed",
                script_path=None,
