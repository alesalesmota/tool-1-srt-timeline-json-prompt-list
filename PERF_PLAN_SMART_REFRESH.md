...
- [ ] Task 1: Add `computeRenderFingerprint()` function + `lastRenderFingerprint` variable
- [ ] Task 2: Guard polling `renderApp()` call with fingerprint check (line 5038 only)
- [ ] Task 3: Make `nicheProjectsPromise` conditional on route
- [ ] Task 4: Verify `boardEpisodesPromise` / `pipeline-board` route status
- [ ] Task 5: Evaluate AbortController need (may be skipped if guard is sufficient)
- [ ] Task 6: Increase polling intervals to 10s idle / 3s active
