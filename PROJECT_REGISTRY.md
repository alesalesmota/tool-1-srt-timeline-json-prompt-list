...
**Currently:** Phases 1-10 are complete, the 2026-03-26 workflow repair is complete, the Drawbridge feedback pass is complete, per-voice TTS pacing control is shipped, the Translation Profiles flow now combines OpenAI API discovery with compact summary cards that open the existing editor modal on click, the global refresh/theme controls now live in the sidebar instead of the topbar, the niche-project `Language setup` / `Provider setup` controls now use explicit button-driven disclosures that preserve open state across rerenders while pausing auto-refresh during active config-field focus, and the episode workflow surface now combines explicit workflow wording, icon-only actions, always-visible inline card status, 1-second active polling, and correct `paused_for_tts` column placement so active narration work no longer looks stuck in `Consistency Guide` or `Draft`. The Tool 1 pipeline is aligned with the intended project-board-first episode workflow and long-form narration now uses explicit app-side TTS chunking plus per-profile pacing presets.
