...
- **AI agent configs** (`config/agents/`) — scene planning, visual bible, video/image prompts (Claude + Codex + OpenAI)
- **Test suite** — 138 tests passing (chunking, cli_runner, translation, tts, video_pipeline, API/service coverage)

### What's Being Worked On
See `IMPLEMENTATION_PLAN.md` and `IMPLEMENTATION_CHECKLIST.md` for the 10-phase plan.

**Currently:** Phases 1-10 are complete, the 2026-03-26 workflow repair is complete, the Drawbridge feedback pass is complete, per-voice TTS pacing control is shipped, the Translation Profiles flow now combines OpenAI API discovery with compact summary cards that open the existing editor modal on click, the workflow-stage provider setup now supports both local CLIs and a shared OpenAI API key with refreshable stage-model discovery, the global refresh/theme controls now live in the sidebar instead of the topbar, the niche-project `Language setup` / `Provider setup` controls now use explicit button-driven disclosures that preserve open state across rerenders while pausing auto-refresh during active config-field focus, and the episode workflow surface now combines explicit workflow wording, icon-only actions, always-visible inline card status, 1-second active polling, correct `paused_for_tts` column placement, visible live-run snapshots inside the episode overlay, chunk-level XTTS telemetry on the card + detail surfaces so active narration work no longer looks frozen during long generations, a dedicated workflow-control panel that can pause at safe boundaries or rerun from specific stages without snapping back to `Consistency Guide`, and a live output-files panel that shows nested artifacts, highlights empty vs populated files, and opens inline previews/downloads without a manual refresh button. The Tool 1 pipeline is aligned with the intended project-board-first episode workflow and long-form narration now uses explicit app-side TTS chunking plus per-profile pacing presets.
...
### Git State
- Branch: `codex/setup-run-action` (active)
...
| Workflow-stage model fields stay manually editable and OpenAI stages share one saved API key | CLI model catalogs can change out of band, so stage model inputs cannot be locked to stale dropdowns; OpenAI-backed scene/style/prompt stages should reuse one settings-level key instead of asking for separate credentials per stage | 2026-03-27 |
...
- Detect CLI model catalogs automatically if/when `codex` or `claude` expose a stable model-list command, so the suggestion datalists can refresh from the installed tool instead of relying on curated defaults
